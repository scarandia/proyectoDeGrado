...
const getFabricantes = async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error al obtener los fabricantes.', detalle: validationError.message  });
    }
};
...
const getFabricanteById = async (req, res) => {
    try {
        ...
        if (!fabricante) {
            return res.status(404).json({ error: 'Fabricante no encontrado.', detalle: validationError.message  });
        }
        res.status(200).json(fabricante);
    } catch (error) {
        res.status(500).json({ error: 'Error al obtener el fabricante.', detalle: validationError.message  });
    }
};
...
const updateFabricante = async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error al actualizar el fabricante.', detalle: validationError.message  });
    }
};
...
const deleteFabricante = async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Error al eliminar el fabricante.' , detalle: validationError.message });
    }
};
